...
<head>
    ...
    <style>
        .empresa-sel-lista { display:flex; flex-direction:column; gap:12px; margin-top:16px; }
        .empresa-sel-item {
        ...
        .empresa-sel-item:hover { box-shadow:0 4px 14px rgba(0,0,0,0.1); }
        .empresa-sel-logo {
        ...
        .empresa-sel-info { flex:1; }
        .empresa-sel-nombre { font-weight:700; font-size:15px; }
        .empresa-sel-meta { font-size:13px; color:#888; margin-top:2px; }
        .empresa-sel-estrella { color:#f5a623; }
        .sin-resenas { text-align:center; color:#aaa; padding:30px; }
    </style>
</head>
<body>
        ...
        <?php while ($emp = $empresas_q->fetch_assoc()): ?>
            <a href="gestionar_resenas.php?empresa=<?= $emp['id_empresa'] ?>" class="empresa-sel-item">
                <?php if (!empty($emp['logo'])): ?>
                    <img src="/guiaempresarial.pe/assets/img/<?= htmlspecialchars($emp['logo']) ?>" class="empresa-sel-logo">
                <?php else: ?>
                ...
                <div class="empresa-sel-info">
                    ...
                    <div class="empresa-sel-meta">
                        ...
                        &nbsp;·&nbsp; <?= $emp['total_resenas'] ?> reseña<?= $emp['total_resenas'] > 1 ? 's' : '' ?>
                    </div>
                </div>
                <span style="color:#0d6efd;font-size:20px;">›</span>
            </a>
        <?php endwhile; ?>
        </div>
        <?php else: ?>
...
if (!$res || $res->num_rows === 0) {
    header("Location: gestionar_resenas.php"); exit;
}
...
<head>
    ...
    <style>
        ...
        .admin-topbar a {
            color:#aad4ff; text-decoration:none; font-size:14px;
        }
        .admin-topbar a:hover { color:#fff; }
        .admin-topbar span { color:#666; }
        .btn-del-resena {
        ...
        .btn-del-resena:hover { background:#c0392b; }
        .alerta-ok {
